...
class ContactDoctorPage extends StatefulWidget {
  ...
  const ContactDoctorPage({super.key, required this.patientId});
...
class _ContactDoctorPageState extends State<ContactDoctorPage> {
  ...
  Future<void> loadPage() async {
    ...
    try {
      final active = await DoctorAppointmentApi.getActiveAppointment(
        widget.patientId,
      );
      ...
      if (active['hasAppointment'] == true) {
        ...
        final list = await DoctorAppointmentApi.getDoctors();
  ...
  Future<void> showEditAppointmentDialog(
    ...
    Future<void> loadEditAvailability(
      ...
      final data = await DoctorAppointmentApi.getAvailability(
        doctorId: appointment['doctorId'] is Map
  ...
  Widget buildCurrentAppointment() {
    ...
    final profile = appointmentData?['doctorProfile'];

    final name = profile?['fullName'] ?? 'Doctor';
  ...
  Widget buildDoctorsList() {
    if (doctors.isEmpty) {
      return const Center(child: Text('No doctors available now'));
    }
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final name = item['fullName'] ?? 'Doctor';
...
class _BookDoctorPageState extends State<BookDoctorPage> {
  ...
  String get doctorUserId {
    final user = widget.doctor['userId'];
  ...
  Widget build(BuildContext context) {
    ...
    final years = doctor['yearsOfExperience'] ?? 0;
